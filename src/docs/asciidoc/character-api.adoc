...
operation::current-character-grow-controller-test/grow-to-random-final-character[snippets='http-request,http-response']

==== 실패

operation::current-character-grow-controller-test/fail_grow-to-next-level-character_valid_level_is_end[snippets='http-response']

=== 캐릭터 최종 성장 이후 초기화

==== 성공

operation::character-initialize-controller-test/initialize-after-final-character[snippets='http-request,http-response']

==== 실패

operation::character-initialize-controller-test/fail_initialize-after-final-character_valid_level_is_final[snippets='http-response']
