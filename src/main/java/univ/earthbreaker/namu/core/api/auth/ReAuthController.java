...
import univ.earthbreaker.namu.core.api.auth.support.RefreshToken;
...
public class ReAuthController {
	...
	@PostMapping("/reissue")
	public ResponseEntity<Void> reissue(@RefreshToken String refreshTokenValue) {
		TokenResult tokenResult = tokenReissueService.reissue(refreshTokenValue);
		return ResponseEntity.ok()
			.header(HttpHeaders.AUTHORIZATION, HttpHeaderUtils.withBearerToken(tokenResult.accessToken()))
			.build();
	}
}
