...
class CurrentCharacterGrowControllerTest extends PresentationTest {
	...
	void growToRandomFinalCharacter() throws Exception {
		// given
		Mockito.when(currentCharacterGrowService.growToFinalRandom(CharacterFixture.MEMBER_NO))
		...
		resultActions
			...
			.andExpect(jsonPath("$.characterNo").value(CharacterFixture.CHARACTER_NO))
			.andExpect(jsonPath("$.mainImageUrl").value(
				System.getProperty(IMAGE_SYSTEM_PROPERTY) + CharacterFixture.CHARACTER_IMAGE_PATH))
			.andExpect(jsonPath("$.backgroundImageUrl").value(
				System.getProperty(IMAGE_SYSTEM_PROPERTY) + CharacterFixture.BACKGROUND_IMAGE_PATH))
			.andExpect(jsonPath("$.scripts").value(CharacterFixture.SCRIPTS));
	...
	void fail_growToMethod_invalid(String exceptionMessage) throws Exception {
		...
		Mockito.doThrow(new IllegalStateException(exceptionMessage))
			...
			.growToEndLevel(PresentationTest.AUTHORIZED_MEMBER_NO);
	...
	void fail_growToMethod_not_found() throws Exception {
		// given
		Mockito.doThrow(CurrentCharacterNotFoundException.notFound(PresentationTest.AUTHORIZED_MEMBER_NO))
			.when(currentCharacterGrowService)
			.growToEndLevel(PresentationTest.AUTHORIZED_MEMBER_NO);
		...
		resultActions
			...
			.andExpect(result ->
				assertThat(result.getResolvedException())
					...
					.hasMessage(
						CurrentCharacterNotFoundException.notFound(PresentationTest.AUTHORIZED_MEMBER_NO).getMessage())
			)
			.andDo(commonExceptionDocumentResultHandler());
	}
	...
	void fail_growToNextLevelCharacter_valid_level_is_end() throws Exception {
		...
		Mockito.doThrow(new IllegalStateException(EXCEPTION_MESSAGE))
			.when(currentCharacterGrowService)
			.growToFinalRandom(PresentationTest.AUTHORIZED_MEMBER_NO);
	...
	void fail_growToNextLevelRandomCharacter_valid_level_is_begin() throws Exception {
		...
		Mockito.doThrow(new IllegalStateException(EXCEPTION_MESSAGE))
			.when(currentCharacterGrowService)
			.growToMiddleLevel(PresentationTest.AUTHORIZED_MEMBER_NO);
