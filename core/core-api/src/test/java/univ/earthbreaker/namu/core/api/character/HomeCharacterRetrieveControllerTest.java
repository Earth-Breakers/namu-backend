...
class HomeCharacterRetrieveControllerTest extends PresentationTest {
	...
	void retrieve() throws Exception {
		// given
		Mockito.when(homeCharacterRetrieveService.retrieveHomeCharacter(CharacterFixture.MEMBER_NO))
		...
		// then
		String expectImagePath = System.getProperty(IMAGE_SYSTEM_PROPERTY) + CharacterFixture.BEGIN_CURRENT_CHARACTER.getTargetCharacterMainImage();
		resultActions
			.andExpect(status().isOk())
			.andExpect(content().contentType(MediaType.APPLICATION_JSON))
			.andExpect(jsonPath("$.characterType").value(DEFAULT.name()))
			.andExpect(jsonPath("$.imageUrl").value(expectImagePath))
			.andExpect(jsonPath("$.requiredExp").value(CharacterFixture.BEGIN_REQUIRED_EXP))
			.andExpect(jsonPath("$.currentExp").value(CharacterFixture.INITIAL_EXP));
