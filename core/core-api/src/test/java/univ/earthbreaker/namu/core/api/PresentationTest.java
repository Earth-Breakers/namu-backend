...
import univ.earthbreaker.namu.test.api.ApiDocsAbstract;
...
public abstract class PresentationTest extends ApiDocsAbstract {
	...
	private HandlerMethodArgumentResolver mockLoginMemberArgumentResolver() throws Exception {
	...
	protected ResultActions whenPostWithAuthorization(String uri, Long pathVariable, Object requestBody) throws
		Exception {
	...
	protected ResultActions whenPostMultipartWithAuthorization(
		String uri,
		Long pathVariable,
		MockPart contentPart,
		MockMultipartFile imageFile
	) throws Exception {
		MockHttpServletRequestBuilder requestBuilder = RestDocumentationRequestBuilders.multipart(uri, pathVariable)
			.file(imageFile)
			.part(contentPart)
			.header(HttpHeaders.AUTHORIZATION, AUTHORIZATION_TYPE_WITH_ACCESS_TOKEN)
			.accept(MediaType.APPLICATION_JSON);
	...
	protected ResultActions whenGetWithAuthorization(String uri, Long pathVariable) throws Exception {
