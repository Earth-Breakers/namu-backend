...
import static univ.earthbreaker.namu.core.domain.post.PostFixture.*;
...
class PostRetrieveControllerTest extends PresentationTest {
	...
	void retrieveAll() throws Exception {
		...
		resultActions
			...
			.andExpect(
				jsonPath("$[0].imageUrl").value(System.getProperty(IMAGE_SYSTEM_PROPERTY) + FIRST_POST.getImagePath()))
			...
			.andExpect(jsonPath("$[1].imageUrl").value(
				System.getProperty(IMAGE_SYSTEM_PROPERTY) + SECOND_POST.getImagePath()));
	...
	void retrieveRelated() throws Exception {
		...
		resultActions
			...
			.andExpect(jsonPath("$.posts[0].imageUrl").value(
				System.getProperty(IMAGE_SYSTEM_PROPERTY) + FIRST_POST.getImagePath()))
			...
			.andExpect(jsonPath("$.posts[1].imageUrl").value(
				System.getProperty(IMAGE_SYSTEM_PROPERTY) + SECOND_POST.getImagePath()));
