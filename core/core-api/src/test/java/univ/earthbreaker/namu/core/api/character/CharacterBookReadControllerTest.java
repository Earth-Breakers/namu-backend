...
class CharacterBookReadControllerTest extends PresentationTest {
	...
	void readAll() throws Exception {
	    // given
		Mockito.when(memberCharacterFinder.findAllBy(CharacterFixture.MEMBER_NO))
		...
		resultActions
			...
			.andExpect(MockMvcResultMatchers.jsonPath("$.totalAcquiredCount").value(TOTAL_ACQUIRED_COUNT))
			.andExpect(MockMvcResultMatchers.jsonPath("$.sectionResponses[0].totalCountOfType").value(TOTAL_COUNT_OF_TYPE))
			.andExpect(MockMvcResultMatchers.jsonPath("$.sectionResponses[0].acquiredCount").value(ACQUIRED_COUNT))
			.andExpect(MockMvcResultMatchers.jsonPath("$.sectionResponses[0].profileResponses[0].thumbnailImageUrl").value(System.getProperty(IMAGE_SYSTEM_PROPERTY) + CharacterFixture.CHARACTER_IMAGE_PATH))
			.andExpect(MockMvcResultMatchers.jsonPath("$.sectionResponses[0].profileResponses[0].isAcquired").value(true))
			.andExpect(MockMvcResultMatchers.jsonPath("$.sectionResponses[0].profileResponses[1].thumbnailImageUrl").value(System.getProperty(IMAGE_SYSTEM_PROPERTY) + CharacterFixture.CHARACTER_IMAGE_PATH))
			.andExpect(MockMvcResultMatchers.jsonPath("$.sectionResponses[0].profileResponses[1].isAcquired").value(false));
	...
	void readDetail() throws Exception {
	    // given
		Mockito.when(characterBookDetailReadService.retrieveDetail(
				CharacterFixture.MEMBER_NO, CharacterFixture.CHARACTER_NO))
			.thenReturn(MEMBER_CHARACTER_1);
		CharacterDetailResponse expect = CharacterDetailResponse.from(MEMBER_CHARACTER_1);

		// when
		ResultActions resultActions = whenGetWithAuthorization(DETAIL_CHARACTER_BOOK_URI, CharacterFixture.CHARACTER_NO);

		// then
		resultActions
			.andExpect(MockMvcResultMatchers.status().isOk())
			.andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON))
			.andExpect(MockMvcResultMatchers.jsonPath("$.characterNo").value(expect.characterNo()))
			.andExpect(MockMvcResultMatchers.jsonPath("$.count").value(expect.count()))
			.andExpect(MockMvcResultMatchers.jsonPath("$.isEndangered").value(expect.isEndangered()))
			.andExpect(MockMvcResultMatchers.jsonPath("$.type").value(expect.type().name()))
			.andExpect(MockMvcResultMatchers.jsonPath("$.gender").value(expect.gender().name()))
			.andExpect(MockMvcResultMatchers.jsonPath("$.name").value(expect.name()))
			.andExpect(MockMvcResultMatchers.jsonPath("$.description").value(expect.description()))
			.andExpect(MockMvcResultMatchers.jsonPath("$.imageUrl").value(expect.imageUrl()));
