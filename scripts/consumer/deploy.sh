...
BASE_SERVER_PATH=$BASE_PATH/namu-consumer-server
...
echo "$CURRENT_TIME > namu-consumer-server-deploy.jar 교체" >> $DEPLOY_LOG
CP_JAR_PATH=$DEPLOY_PATH$JAR_NAME
APPLICATION_JAR_NAME=namu-consumer-server-deploy.jar
...
SENTRY_ENVIRONMENT=$SPRING_PROFILES_ACTIVE nohup java -jar \
  ...
  -DBOOTSTRAP_SERVER_PATH="$BOOTSTRAP_SERVER_PATH" \
